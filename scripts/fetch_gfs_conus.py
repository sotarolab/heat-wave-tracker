...
Downloads T2m and Td2m from NOAA AWS via Herbie, computes heat index,
and saves to data/conus_heat_tracker.nc. Defaults to every 2 hours (61
steps, about 40 MB), a middle ground between the original 4 slots/day
and full hourly (121 steps, about 80 MB, over GitHub's 50 MB soft
file-size limit).

This script runs locally. Commit the output file so the deploy host
serves from it without a cold-start download.

Usage
-----
    # latest available GFS cycle (default: 61 steps, F000-F120 every 2h)
    python scripts/fetch_gfs_conus.py

    # specific init time
    python scripts/fetch_gfs_conus.py --init "2026-07-02 00:00"

    # full hourly (121 steps, about 80 MB, exceeds GitHub's 50 MB soft limit)
    python scripts/fetch_gfs_conus.py --step 1

    # coarser/faster fetch (21 steps, F000-F120 every 6h), smaller file
